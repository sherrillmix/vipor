...
```{r adjust-examples, tidy=TRUE, results='hide'}
...
sapply(names(dat),function(label) {
	...
	offsets <- list(
		'Default'=offsetX(y),
		'Adjust=2'=offsetX(y, adjust=2),
		'Adjust=.1'=offsetX(y, adjust=0.1),
		'Width=10%'=offsetX(y, width=0.1) 
	)  
...
```{r other-methods, tidy=TRUE, results='hide'}
library(beeswarm)
par(mfrow=c(4,1), mar=c(2.5,3.1, 1.2, 0.5),mgp=c(2.1,.75,0),cex.axis=1.2,cex.lab=1.2,cex.main=1.2)
sapply(names(dat),function(label) {
	y<-dat[[label]]
