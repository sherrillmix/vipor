...
%\VignetteDepends{lattice,beeswarm,ggplot2}
...
An example using the \code{EuStockMarkets} data from the \pkg{datasets} package:
...
Another example using 8000 entries of the \code{diamonds} data from the \pkg{ggplot2} package. Here \code{beeswarm} takes too long to run and is omitted:
